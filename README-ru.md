<div align = "center">

<img alt="значок" src="./Sandbox/Resources/Images/Frostnux.png" style = "width: 20%; height: auto;">

</div>
<div align = "center">

# Frostnux · Остров
<strong> C++ / OpenGL как встроенная среда разработки с открытым исходным кодом при запуске. </strong>

<em>Русское название: <strong>Остров</strong> (официальное название на русском языке). Название репозитория остается <code>Frostnux</code>.</em>
</div>

<div align = "center">

[English](./README.md)
[中文](./README-zh.md)
[Pусский](./README-ru.md)
[Magyar](./README-hu.md)
</div>
<div align = "center">
<strong> Выковано при нуле. Заточено в битах. </strong>
</div>

## Скриншот работы

<div align = "center">
<img src = ".docs/assert/RuntimeScreenshot.png" alt = "Скриншот работы">
</div>
<div align="center">
<em> Скриншот реального экрана работы IDE во время <code>компиляции</code> </em>
</div>

## Что такое Frostnux
Frostnux - это небольшая интегрированная среда разработки, созданная школьником дома, которая предлагает альтернативный вариант для написания кода,

Текущая версия 2026.3.3, поддерживается язык C++

## Лицензия

Frostnux выпускается на основе протокола Apache, Подробности см. в файле [`LICENSE`](./LICENSE).
